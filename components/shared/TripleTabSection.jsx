...
export function TripleTabSection({ tabsData }) {
  ...
  return (
    <Tabs value='1'>
      ...
      >
        ...
        >
          {tabsData[0]?.top?.map(({ label, value }) => (
            <Tab
              key={value}
              value={value}
            ...
            >
              <div className='flex items-center gap-x-6 gap-y-2 md:flex-col md:justify-center 2xl:flex-row'>
                <div
                  className={`py-2 sm:text-lg text-sm text-gray-750 ${bitter.className} ${activeTab === value ? 'text-gray-50 duration-1000' : ''}`}
                >
                  {parse(label)}
                </div>
              </div>
        ...
        <TabsHeader
          className={`p-1 rounded-full bg-[#F4F4F4] md:p-1 flex items-center justify-center mt-4 w-[100%] mx-auto`}
          indicatorProps={{
            className: 'bg-[#1B2639] shadow-none rounded-full',
          }}
        >
          {tabsData[0]?.middle?.map(({ label, value }) => (
            <Tab
              key={value}
              value={value}
              className='md:p-5 p-2 !text-center '
            ...
            >
              <div className='flex items-center gap-x-6 gap-y-2 md:flex-col md:justify-center 2xl:flex-row'>
                <div
                  className={`py-2 sm:text-lg text-sm text-gray-750 ${bitter.className} ${activeTab === value ? 'text-gray-50' : ''}`}
                >
                  {parse(label)}
                </div>
              </div>
        ...
        >
          {tabsData[0]?.bottom?.map(({ label, value }) => (
            <Tab
              key={value}
              value={value}
              className='md:p-5 p-2 !text-center '
      ...
      >
        <TabsBody className='p-0 m-0'>
          {tabsData[0]?.details?.map(({ label, content, value, img, slug }) => (
